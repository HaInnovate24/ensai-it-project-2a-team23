...
- la récupération et à la mise à jour des données provenant de `l’API ILOSTAT` ;
...
- **P1** **— Chef de projet & Qualité :** coordination, suivi et qualité des livrables.

- **P2 — Data Engineer :** collecte, traitement et préparation des données.

- **P3 — Base de données & DAO :** modélisation, stockage et accès aux données.

- **P4 — Backend / API :** développement des services et endpoints de l’`API`.

- **P5 — Front & DataViz** : interface utilisateur et visualisation des données.
...
- **Gestion des utilisateurs** : `Utilisateur` (authentification et rôles), `JournalActivite` (traçabilité des actions sensibles) et `Rapport` (rapports PDF générés) ;
- **Référentiels ILOSTAT** : `Pays`, `Indicateur`, `Sexe`, `Age` et `Occupation`, qui décrivent les dimensions d’analyse ;
- **Données** : `Observation`, classe centrale reliant chaque valeur mesurée à ses cinq dimensions, et `ImportDonnees`, qui trace les chargements depuis l’`API`.
...
Le modèle physique de données traduit le diagramme de classes en schéma relationnel. Il réunit l’ensemble des tables et met en évidence les **clés primaire**s, les **clés étrangères** et les relations entre elles.
