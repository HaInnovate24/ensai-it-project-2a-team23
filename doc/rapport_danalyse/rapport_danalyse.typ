...
#show heading.where(level: 3): it => {
  block(above: 1.1em, below: 0.5em)[
    #text(size: 11.5pt, weight: "bold", style: "italic")[
      #counter(heading).display() #h(0.4em) #it.body
    ]
  ]
}
...
Le premier choix décisif a concerné l'architecture globale de l'outil. Bien que le cahier des charges laisse la possibilité de se limiter au développement de l'API, nous avons pris le parti de développer la solution la plus complète : une API couplée à une interface graphique (front-end). Cette approche nous paraît indispensable pour répondre à l'objectif principal du projet, c'est-à-dire rendre des volumes importants de statistiques accessibles, exploitables et visuellement clairs pour l'utilisateur final.
...
#figure([
#box(image("authentification.png", width: 100.0%))
], caption: figure.caption(
position: bottom, 
[
Diagramme de séquence (authentification)
...
#figure([
#box(image("donnees.png", width: 100.0%))
], caption: figure.caption(
position: bottom, 
[
Diagramme de séquence (données analytiques)
