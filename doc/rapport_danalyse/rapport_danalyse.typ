...
<questionnements-et-choix>
Lors de la phase de conception de LaborScope, plusieurs réflexions ont été menées afin de définir les orientations techniques et fonctionnelles du projet. Le premier choix décisif a concerné l'architecture globale de l'outil. Bien que le cahier des charges laisse la possibilité de se limiter au développement de l'#NormalTok("API");, nous avons pris le parti de développer la solution la plus complète : une #NormalTok("API"); couplée à une interface graphique (front-end). Cette approche nous paraît indispensable pour répondre à l'objectif principal du projet, c'est-à-dire rendre des volumes importants de statistiques accessibles, exploitables et visuellement clairs pour l'utilisateur final. Une autre réflexion majeure a porté sur le système d'authentification et la répartition des droits d'accès. Plutôt que d'imposer la création d'un compte de manière stricte dès l'ouverture de l'application, nous avons opté pour une approche progressive. L'objectif est de démontrer l'utilité de notre outil dès la première visite. Ainsi, nous avons décidé de laisser la fonctionnalité d'analyse temporelle de l'emploi (#NormalTok("F3");) en accès libre. Un visiteur non connecté pourra donc interroger la base de données, générer un graphique et observer l'évolution d'un indicateur sans restriction. En revanche, l'accès aux fonctionnalités d'analyse plus avancées (comparaison géographique, croisement multi-indicateurs, cartographie ou génération de rapports) nécessitera obligatoirement une authentification. Ce choix stratégique permet d'offrir un aperçu concret des capacités de l'application pour susciter l'intérêt de l'utilisateur, tout en l'incitant à s'inscrire pour bénéficier d'une expérience complète. Nous avons fait le choix d'orienter l'application vers une logique marketing et commerciale afin d'attirer et de fidéliser de potentiels clients comme dans une vraie application. Cette dynamique permet d'enrichir notre base de données clients et s'intègre parfaitement avec le système de gestion des rôles (#NormalTok("Utilisateurs"); et #NormalTok("Administrateurs");) mis en place pour administrer l'application.

= Description des fonctionnalités
<description-des-fonctionnalités>
Le projet comporte différentes fonctionnalités qui se répartissent entre fonctionnalités obligatoires et optionnelles. Il y a 6 fonctionnalités obligatoires ainsi que 2 fonctionnalités optionnelles.

La première fonctionnalité de cette application consiste en la récupération et le stockage automatisés des données. Ainsi, l'application interroge automatiquement et de façon périodique #NormalTok("l’API ILOSTAT"); et récupère les indicateurs souhaités sur plusieurs pays. Ensuite vient le traitement des données, les fichiers bruts sont triés pour ne conserver que les champs jugés essentiels au bon fonctionnement de l'application. Ces données nettoyées sont ensuite sauvegardées dans la base de données locale et deviennent enfin prêtes à être analysées.

Une autre fonctionnalité doit permettre l'analyse temporelle des données extraites et stockées. L'utilisateur pourra sélectionner un indicateur, définir une fenêtre de temps et afficher les résultats sous forme graphique. Concrètement, après la sélection des paramètres, l'#NormalTok("API"); calcule cette évolution temporelle et l'interface permet d'observer les dynamiques du marché du travail.
...
- #strong[P4 --- Backend / API :] développement des services et endpoints de l'#NormalTok("API");.
...
<organisation-du-travail>
Concernant l'organisation du travail, notre équipe s'appuie sur plusieurs outils collaboratifs afin d'assurer une communication fluide et une répartition efficace des tâches. Pour nos échanges quotidiens, nous disposons d'un groupe de discussion sur #NormalTok("WhatsApp");, ce qui nous permet de débattre de nos idées et de faire des points réguliers sur l'avancement du projet. En parallèle, nous utilisons #NormalTok("Google Docs"); pour la rédaction et le partage de nos documents communs garantissant ainsi à chaque membre un accès simultané pour la lecture et la modification des fichiers. La gestion de projet et le suivi des tâches sont quant à eux centralisés sur la plateforme #NormalTok("Notion");. Cet espace de travail nous permet de répertorier l'intégralité des actions à mener. Chaque tâche y est qualifiée selon plusieurs critères : son statut (à faire, en cours ou terminé), la phase du projet à laquelle elle se rattache, son niveau d'importance, ainsi que le membre du groupe qui en est responsable. Cette méthode structure notre organisation, offre une vision claire de la progression globale et permet à chacun de savoir précisément ce qu'il doit accomplir. Enfin, concernant la partie développement, nous codons notre application sur #NormalTok("VSCode"); et nous nous appuyons sur #NormalTok("GitHub"); pour centraliser notre travail. Ce répertoire commun est très utile : il sécurise notre collaboration, nous permet de coder en parallèle, de conserver tout l'historique de nos modifications et de résoudre facilement les éventuels conflits de versions.
...
Le profil #NormalTok("Utilisateur"); est le rôle standard. Il est strictement limité à l'authentification ainsi qu'à la consultation des analyses et des visualisations telles que l'analyse temporelle, la comparaison géographique ou encore la comparaison multi-indicateurs. Il bénéficie également d'outils tels que la cartographie et la génération de rapports. Les limites du profil #NormalTok("Utilisateur"); s'opposent donc aux compétences du profil #NormalTok("Administrateur"); qui a un rôle dédié aux tâches de gestion de l'application. Il est le seul à pouvoir modifier ou supprimer certaines données et à gérer l'accès des profils #NormalTok("Utilisateur");. De plus, la flèche d'héritage reliant l'Administrateur à l'Utilisateur indique que l'Administrateur hérite automatiquement de toutes les fonctionnalités auxquelles l'Utilisateur a accès.

#figure_block("images/diagramme_cas_utilisation.drawio.png", "Diagramme de cas d'utilisation", w: 10cm, h: 4cm)
== Diagramme d'activité
<diagramme-dactivité>
Le diagramme présenté permet de caractériser les choix offerts à l'utilisateur lorsqu'il ouvre l'application. Dans un premier temps, il va soit se connecter, soit créer un compte s'il n'en a pas.
...
<diagramme-de-classes>
Le diagramme de classes formalise la structure de données de #NormalTok("LaborScope");. Il s'organise en trois ensembles :
...
- #strong[Données] : #NormalTok("Observation");, classe centrale reliant chaque valeur mesurée à ses cinq dimensions, et #NormalTok("ImportDonnees");, qui trace les chargements depuis l'#NormalTok("API");.
...
<diagramme-de-séquences-authentification>
On retrouve dans cette partie le diagramme de séquence concernant la fonctionnalité de l'authentification. Ce diagramme illustre l'ordre chronologique des interactions entre la personne qui tente de se connecter et les trois couches du système que sont l'interface, l'#NormalTok("API"); ainsi que la base de données. L'objectif de ce diagramme est de visualiser comment l'application sépare, en fonction du profil renseigné, les droits de chaque profil (#NormalTok("Utilisateur"); ou #NormalTok("Administrateur");). Le processus d'authentification est initié par une personne qui saisit son nom et son mot de passe directement sur l'interface. Afin de maintenir la sécurité de l'architecture, l'Interface ne communique jamais directement avec les données, elle formule une demande d'authentification qu'elle transmet au Serveur. C'est l'#NormalTok("API"); qui porte le rôle de la vérification. En effet, le Serveur interroge la Base de données locale pour vérifier si les informations transmises correspondent à un compte existant. Si le compte existe, la Base de données retourne à l'#NormalTok("API"); l'identité de la personne ainsi que son rôle exact, à savoir s'il est simple Utilisateur ou bien Administrateur. En fonction de la réponse de la Base de données, l'application fait face à trois scénarios distincts selon le niveau d'authentification. En effet, selon que le profil renseigné soit utilisateur ou Administrateur, le cahier des charges nous indique que l'utilisateur ne doit pas bénéficier de tous les accès. Voici la liste des trois scénarios :

- Si le rôle détecté est #NormalTok("Utilisateur");, l'#NormalTok("API"); confirme la réussite de l'authentification à l'Interface et accorde ensuite un accès restreint à cet Utilisateur (consultation des analyses et des visualisations)
- Si le rôle détecté est #NormalTok("Administrateur");, l'#NormalTok("API"); confirme la réussite de l'authentification à l'Interface et accorde ensuite tous les accès de l'application à ce profil
- Si la vérification échoue, l'#NormalTok("API"); notifie le refus de connexion à l'Interface et bloque l'accès à l'application en affichant un message d'erreur.
...
<diagramme-de-séquences-données-analytiques>
Cette partie présente le diagramme de séquence qui regroupe les fonctionnalités d'analyse de l'application telles que l'analyse temporelle, la comparaison géographique d'un indicateur ou encore la comparaison multi-indicateurs. Même si les calculs statistiques cachés derrière ces fonctionnalités diffèrent, le cheminement informatique reste similaire entre ces fonctionnalités.

Ainsi, pour ces fonctionnalités, le processus est déclenché par l'utilisateur qui, depuis l'Interface sélectionne les paramètres d'analyse : indicateur, pays, période de temps. Une fois la demande d'analyse validée, l'Interface formule une requête qu'elle envoie à l'#NormalTok("API");. À la réception de cette requête, l'#NormalTok("API"); prend le relais concernant la gestion des données et interroge la Base de données locale avec les critères de filtrage renseignés par l'utilisateur. Ensuite la Base de données répond à cette requête en retournant les données correspondantes. La dernière étape consiste en la préparation et l'affichage des résultats. Pour cela, l'#NormalTok("API"); nettoie les données brutes et calcule les statistiques demandées par l'utilisateur avant de renvoyer ces données formatées à l'Interface. À la réception de ces données finales, l'Interface les utilise pour construire le graphique approprié et l'affiche à l'utilisateur.

#figure_block("images/donnees.png", "Diagramme de séquence - analyse ", w: 14cm, h: 12cm)
== Diagramme de packages
<diagramme-de-packages>
Le diagramme de packages nous permet de voir comment les différentes couches du code interagissent entre elles. La première couche est le #NormalTok("frontend");, et permet à l'utilisateur d'interagir avec l'application. Il pourra se connecter et ensuite naviguer dans l'application. La deuxième couche est l'#NormalTok("API");, qui va transmettre les demandes, commandes de l'utilisateur vers le programme. La couche de services va réceptionner ces demandes pour ensuite réaliser des calculs. Cette couche comprend tous les objets, classes relatives aux utilisateurs et calculs à effectuer. Elle interagit avec la dernière couche qui est la couche de données. Les données sont récupérées et parsées, pour être envoyées à la #NormalTok("DAO");. Les données seront alors traitées pour ensuite être stockées localement dans une base de données #NormalTok("PostgreSQL");. Ces données seront donc exploitées afin d'effectuer les calculs dans la couche de services. Enfin, on pourra réaliser des visuels graphiques dans la couche de services, qui apparaîtront directement dans le #NormalTok("frontend");.
...
== Les DAO
<les-dao>
Le rôle de la couche #NormalTok("DAO"); (#strong[Data Access Object]) est de récupérer les données de la base de données locale. Cette couche assure le relai technique entre la base de données locale et la couche de services. Concernant ses dépendances, ce composant est totalement indépendant des autres parties de l'application. Son seul lien direct est avec la base de données locale, qu'il a justement pour mission de manipuler. À l'inverse, le serveur principal dépend entièrement de lui pour pouvoir retrouver ces informations. Ainsi, de par son importance technique, ce composant est à développer avec une priorité absolue avec un temps de développement estimé à 40H.

== La couche de services
<la-couche-de-services>
La couche de services contiendra les classes et objets métiers. C'est cette couche qui récupérera les requêtes via l'#NormalTok("API"); et qui les traitera à l'aide des fonctions implémentées. Les principales fonctionnalités seront l'affichage de l'évolution d'indicateurs, la comparaison entre pays et la comparaison de plusieurs indicateurs. Elle interagit avec la #NormalTok("DAO"); pour la récupération de données utiles aux calculs. Le temps de développement est estimé à 35H.

== L'API
<lapi>
L'#NormalTok("API"); (Application Programming Interface) va nous permettre de lier le #NormalTok("frontend"); et les couches métier. Elle reçoit les requêtes #NormalTok("HTTP");, les valide et communique avec les classes métier nécessaires. Elle ne sert pas à réaliser les calculs mais permet de gérer les appels du #NormalTok("frontend");, notamment sur l'authentification et l'exécution de fonctionnalités. Cette articulation sera faite à l'aide de contrôleurs. Une étape de validation est nécessaire sur l'authentification afin de valider ou refuser l'accès à un utilisateur. La couche #NormalTok("API"); sera développée avec #NormalTok("FastAPI"); et le temps de développement est estimé à 20H.

== Le frontend
<le-frontend>
Le #NormalTok("frontend"); est la couche d'interaction entre l'utilisateur et l'application. Développé avec #NormalTok("Streamlit");, il permet à l'utilisateur de se connecter, de choisir des fonctionnalités et de visualiser les résultats des analyses. Il communique avec le #NormalTok("backend"); pour traiter les demandes. Le temps de développement prévu est de 15H. #box(image("images/composant.png", width: 4.88542in))

= Conclusion
<conclusion>
Ce dossier d'analyse a permis de poser les fondations du projet #NormalTok("LaborScope");. En partant du cahier des charges, nous avons défini les fonctionnalités attendues, justifié nos choix techniques et fonctionnels, puis modélisé l'application à l'aide des différents diagrammes UML (cas d'utilisation, activité, classes, séquences et packages). Cette phase de conception nous offre une vision claire de l'architecture retenue : une #NormalTok("API"); développée avec #NormalTok("FastAPI");, une couche de services portant la logique métier, une base de données #NormalTok("PostgreSQL"); accessible via les #NormalTok("DAO"); et une interface utilisateur réalisée avec #NormalTok("Streamlit");.

Au-delà de la modélisation, cette étape a structuré notre organisation collective grâce à une répartition précise des rôles et à une planification détaillée présentée dans le diagramme de Gantt. Nous disposons ainsi d'un cadre de travail solide qui guidera la phase de développement à venir. Bien que certains aspects puissent encore évoluer au fil de la réalisation, ce dossier constitue une base sur laquelle nous nous appuierons pour construire une solution simple, évolutive et réellement utile à l'analyse des données du marché du travail. Les prochaines étapes seront consacrées à l'implémentation des fonctionnalités, à leur test et à leur intégration, dont les résultats seront présentés dans le dossier final.
